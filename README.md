...
We consider a set of linear strategies of the form $R^{w}_{t+1} = w_t R_{t+1}$. Pick a **position matrix** $L$ that turns signals into weights $w_t = S_t^\top L$. The framework nests the traditional and more familiar cases of simple factor portfolios and long-short portfolios.
